...
4. **Layer the features** -- which are foundational? Which depend on others? List layers top-first (topmost/usable first, foundational last).
...
7. **Write the spec file** -- a single `nil/root.clj` whose last form is `(nilify/root [...])`
...
**Think foundation-first, but list top-first.** Design the domain model / core logic first, then infrastructure, then UI/API -- but in the tree the topmost/usable layer is listed first and the foundational layers last.
...
**`:desc` uses `nilify/prompt`** for multi-line descriptions (joins strings with spaces):
```clojure
:desc (nilify/prompt
...
**Tree structure:** a system's body contains either layers directly (simple systems) or subsystems (composed systems). Subsystems own the typed `:provides` routes and declare `:uses` for siblings. Layers are ordered top-first. Features carry `:desc` and `:internals`.
